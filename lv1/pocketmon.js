...
function solution(nums) {
    ...
    let answer = Array.from(pkm);
    return answer.length >= nums.length / 2 ? nums.length / 2 : answer.length; // 최대로 가질 수 있는 포켓몬의 수는 배열길이 / 2 이므로 같은 종류들은 제외시킨 후 그 수가 
}                                                                              // 배열길이 / 2 보다 크다면 최대 갯수를, 아니면 answer의 길이가 최대 갯수가 된다.


// 다른사람의 코드
function solution(nums) {
    const max = nums.length / 2;
    const arr = [...new Set(nums)]; // 저렇게 Set 하고 나서 전개연산자로 묶어도 되는구나,,,
